...
def mostrar_agenda():
    ...
    <head>
        <style>
            body {{
                font-family: Arial;
                background-color: #E6F3FF; /* Pantone 290 C */
                padding: 20px;
            }}
            ...
            .botao {{
                background-color: #A4DDED; /* Pantone secundário */
                border: none;
                padding: 8px 12px;
                border-radius: 6px;
                cursor: pointer;
    ...
    for dia in dias_semana:
        ...
        for i, slot in enumerate(agenda[dia]):
            ...
            # ALUNA
            if "usuario" in session and usuario["tipo"] == "aluna":
            ...
            # PERSONAL
            if "usuario" in session and usuario["tipo"] == "profissional":
                html += f"<a href='/resetar/{dia}/{i}'><button class='botao'>Resetar</button></a>"
...
def resetar(dia, index):
    ...
    agenda[dia][index]["alunas"] = []  # 🔥 garante limpeza total
